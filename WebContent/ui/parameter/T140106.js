	...
	var delMenu = {
			...
			handler : function() {
				if(grid.getSelectionModel().hasSelection()) {
					...
					showConfirm('确定要删除该工具型号吗？工具型号id：' + railConstOrgid,grid,function(bt) {
						if(bt == "yes") {
							Ext.Ajax.requestNeedAuthorise({
								url: 'T140106Action.asp?method=delete',
								success: function(rsp,opt) {
//									alert(JSON.stringify(rsp)+"  >><<  "+JSON.stringify(opt));
									var rspObj = Ext.decode(rsp.responseText);
									if(rspObj == "00"){
//										Ext.MessageBox.alert('操作提示', railConstOrgid+'工具型号已成功删除!');
										showSuccessMsg('工具型号已成功删除!',grid);//showErrorMsg(action.result.msg,addMhForm)
	            ...
	            Ext.Ajax.request({
					...
					success : function(rsp, opt) {
						...
						if(rspObj.success) {
				            ...
				            Ext.getCmp('modelNameUp').setValue(rspObj.msg.modelName);
	            ...
	            handler: function() {
	            	if(updTermForm.getForm().isValid()) {
						...
						updTermForm.getForm().submit({
							url: 'T140106Action.asp?method=edit',
							waitMsg: '正在保存，请稍后......',
							...
							params: {
								...
								modelName: Ext.getCmp('modelNameUp').getValue(),
								enableStatus:asVal
							}
						});
					}
		...
		var openMenu = {
				...
				handler : function() {
					if(grid.getSelectionModel().hasSelection()) {
		        		var rec = grid.getSelectionModel().getSelected();
						var railConstOrgid = rec.get('id');
						showConfirm('确定要开启该工具型号吗？工具型号id：' + railConstOrgid,grid,function(bt) {
							if(bt == "yes") {
								Ext.Ajax.requestNeedAuthorise({
									url: 'T140106Action.asp?method=open',
									success: function(rsp,opt) {
		...
		var stopMenu = {
				...
				handler : function() {
					if(grid.getSelectionModel().hasSelection()) {
		        		var rec = grid.getSelectionModel().getSelected();
						var railConstOrgid = rec.get('id');
						showConfirm('确定要停用该工具型号吗？工具型号id：' + railConstOrgid,grid,function(bt) {
							if(bt == "yes") {
								Ext.Ajax.requestNeedAuthorise({
									url: 'T140106Action.asp?method=close',
									success: function(rsp,opt) {
		            ...
		            Ext.Ajax.request({
						...
						success : function(rsp, opt) {
							...
							if(rspObj.success) {
					            ...
					            Ext.getCmp('modelNameDet').setValue(rspObj.msg.modelName);
	...
	var excelQueryForm = new Ext.form.FormPanel({
		...
		buttons: [{
			...
			handler: function() {
				...
				var modelName=  Ext.getCmp('modelNamefind').getValue();
				var param = "?a=1";
				if(modelName){
					param = param + "&modelName="+modelName;
				}
				window.location.href = Ext.contextPath +"/exportExcelT140106.asp"+param;
				excelDown.hide();
...
//			btn.disable();
			frm.submit({
				url: 'T140106Action.asp?method=add',
				waitTitle : '请稍候',
...
//	menuArr.push('-');
	...
	var termStore = new Ext.data.Store({
        proxy: new Ext.data.HttpProxy({
            url: 'gridPanelStoreAction.asp?storeId=toolModel'
        }),
    ...
    termStore.on('beforeload', function() {
    	Ext.apply(this.baseParams, {
            start: 0,
            modelName: Ext.getCmp('modelNamefind').getValue()
        });
    });
	
	var termColModel = new Ext.grid.ColumnModel([
    	new Ext.grid.RowNumberer(),
		{id: 'id',header: '工具型号id',dataIndex: 'id',sortable: true,width: 120,hidden : true},
		{header: '工具型号名称',dataIndex: 'modelName',sortable: true,width: 120},
		{header: '工具型号状态',dataIndex: 'enableStatus',sortable: true,renderer:oprState,width: 120},
