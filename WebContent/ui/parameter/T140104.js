	...
	var delMenu = {
			...
			handler : function() {
				if(grid.getSelectionModel().hasSelection()) {
					...
					var toolTypeName = rec.get('toolTypeName');
					showConfirm('确定要删除该工具分类吗？工具分类名称：' + toolTypeName,grid,function(bt) {
						if(bt == "yes") {
							Ext.Ajax.requestNeedAuthorise({
								url: 'T140104Action.asp?method=delete',
								success: function(rsp,opt) {
//									alert(JSON.stringify(rsp)+"  >><<  "+JSON.stringify(opt));
									var rspObj = Ext.decode(rsp.responseText);
									if(rspObj == "00"){
//										Ext.MessageBox.alert('操作提示', railConstOrgid+'工具分类已成功删除!');

										showSuccessMsg('工具分类已成功删除!',grid);//showErrorMsg(action.result.msg,addMhForm)
	            ...
	            Ext.Ajax.request({
					...
					success : function(rsp, opt) {
						...
						if(rspObj.success) {
				            ...
				            Ext.getCmp('toolTypeNameUp').setValue(rspObj.msg.toolTypeName);
	            ...
	            handler: function() {
	            	if(updTermForm.getForm().isValid()) {
						...
						updTermForm.getForm().submit({
							url: 'T140104Action.asp?method=edit',
							waitMsg: '正在保存，请稍后......',
							...
							params: {
								...
								toolTypeName: Ext.getCmp('toolTypeNameUp').getValue(),
								enableStatus:asVal,
								txnId: '140104',
								subTxnId: '02'
							}
		...
		var openMenu = {
				...
				handler : function() {
					if(grid.getSelectionModel().hasSelection()) {
						...
						var railConstOrgid = rec.get('id');
						var toolTypeName = rec.get('toolTypeName');
						showConfirm('确定要启用该工具分类吗？工具分类名称：' + toolTypeName,grid,function(bt) {
							if(bt == "yes") {
								Ext.Ajax.requestNeedAuthorise({
									url: 'T140104Action.asp?method=open',
									success: function(rsp,opt) {
		...
		var stopMenu = {
				...
				handler : function() {
					if(grid.getSelectionModel().hasSelection()) {
						...
						var railConstOrgid = rec.get('id');
						var toolTypeName = rec.get('toolTypeName');
						showConfirm('确定要停用该工具分类吗？工具分类名称：' + toolTypeName,grid,function(bt) {
							if(bt == "yes") {
								Ext.Ajax.requestNeedAuthorise({
									url: 'T140104Action.asp?method=close',
									success: function(rsp,opt) {
		            ...
		            Ext.Ajax.request({
						...
						success : function(rsp, opt) {
							...
							if(rspObj.success) {
					            ...
					            Ext.getCmp('toolTypeNameDet').setValue(rspObj.msg.toolTypeName);
	...
	var excelQueryForm = new Ext.form.FormPanel({
		...
		buttons: [{
			...
			handler: function() {
				...
				var toolTypeName =  Ext.getCmp('toolTypeNamefind').getValue();
				var param = "?a=1";
				if(toolTypeName){
					param = param + "&toolTypeName="+toolTypeName;
				}
				window.location.href = Ext.contextPath +"/exportExcelT140104.asp"+param;
				excelDown.hide();
...
//			btn.disable();
			frm.submit({
				url: 'T140104Action.asp?method=add',
				waitTitle : '请稍候',
...
//	menuArr.push('-');
	...
	var termStore = new Ext.data.Store({
        proxy: new Ext.data.HttpProxy({
            url: 'gridPanelStoreAction.asp?storeId=toolType'
        }),
    ...
    termStore.on('beforeload', function() {
    	Ext.apply(this.baseParams, {
            start: 0,
            toolTypeName: Ext.getCmp('toolTypeNamefind').getValue()
        });
    });
	
	var termColModel = new Ext.grid.ColumnModel([
    	new Ext.grid.RowNumberer(),
		{id: 'id',header: '工具分类id',dataIndex: 'id',sortable: true,width: 120,hidden : true},
		{header: '工具分类名称',dataIndex: 'toolTypeName',sortable: true,width: 120},
		{header: '工具分类状态',dataIndex: 'enableStatus',sortable: true,renderer:enableStatusRender,width: 120},
