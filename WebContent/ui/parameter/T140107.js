	...
	var delMenu = {
			...
			handler : function() {
				if(grid.getSelectionModel().hasSelection()) {
					...
					var equipTypeName = rec.get('equipTypeName');
					showConfirm('确定要删除该终端类型吗？终端类型名称：' + equipTypeName,grid,function(bt) {
						if(bt == "yes") {
							Ext.Ajax.requestNeedAuthorise({
								url: 'T140107Action.asp?method=delete',
								success: function(rsp,opt) {
//									alert(JSON.stringify(rsp)+"  >><<  "+JSON.stringify(opt));
									var rspObj = Ext.decode(rsp.responseText);
									if(rspObj == "00"){
//										Ext.MessageBox.alert('操作提示', railConstOrgid+'终端类型已成功删除!');
										showSuccessMsg('终端类型已成功删除!',grid);//showErrorMsg(action.result.msg,addMhForm)
	            ...
	            Ext.Ajax.request({
					...
					success : function(rsp, opt) {
						...
						if(rspObj.success) {
				            ...
				            Ext.getCmp('equipTypeNameUp').setValue(rspObj.msg.equipTypeName);
	            ...
	            handler: function() {
	            	if(updTermForm.getForm().isValid()) {
						...
						updTermForm.getForm().submit({
							url: 'T140107Action.asp?method=edit',
							waitMsg: '正在保存，请稍后......',
							...
							params: {
								...
								equipTypeName: Ext.getCmp('equipTypeNameUp').getValue(),
							    enableStatus:asVal,
							    txnId: '140107',
								subTxnId: '02'
							}
		...
		var openMenu = {
				...
				handler : function() {
					if(grid.getSelectionModel().hasSelection()) {
						...
						var railConstOrgid = rec.get('id');
						var equipTypeName = rec.get('equipTypeName');
						showConfirm('确定要启用该终端类型吗？终端类型名称：' + equipTypeName,grid,function(bt) {
							if(bt == "yes") {
								Ext.Ajax.requestNeedAuthorise({
									url: 'T140107Action.asp?method=open',
									success: function(rsp,opt) {
//									   Ext.MessageBox.alert('操作提示', '处理成功,已启用');
		...
		var stopMenu = {
				...
				handler : function() {
					if(grid.getSelectionModel().hasSelection()) {
						...
						var railConstOrgid = rec.get('id');
						var equipTypeName = rec.get('equipTypeName');
						showConfirm('确定要停用该终端类型吗？终端类型名称：' + equipTypeName,grid,function(bt) {
							if(bt == "yes") {
								Ext.Ajax.requestNeedAuthorise({
									url: 'T140107Action.asp?method=close',
									success: function(rsp,opt) {
		            ...
		            Ext.Ajax.request({
						...
						success : function(rsp, opt) {
							...
							if(rspObj.success) {
					            ...
					            Ext.getCmp('equipTypeNameDet').setValue(rspObj.msg.equipTypeName);
	...
	var queryForm = new Ext.form.FormPanel({
		...
		items: [{
			xtype: 'textfield',
			id: 'equipTypeNamefind',
			name: 'equipTypeNamefind',
			fieldLabel: '终端类型名称',
			maxLength: 80,
			width:200
		}]
	...
	var excelQueryForm = new Ext.form.FormPanel({
		...
		buttons: [{
			...
			handler: function() {
				...
				var  equipTypeName = Ext.getCmp('equipTypeNamefind').getValue();
				var param = "?a=1";
				if(equipTypeName){
					param = param + "&equipTypeName="+equipTypeName;
				}
				window.location.href = Ext.contextPath +"/exportExcelT140107.asp"+param;
				excelDown.hide();
...
//			btn.disable();
			frm.submit({
				url: 'T140107Action.asp?method=add',
				waitTitle : '请稍候',
...
//	menuArr.push('-');
	...
	var termStore = new Ext.data.Store({
        proxy: new Ext.data.HttpProxy({
            url: 'gridPanelStoreAction.asp?storeId=accessEquipType'
        }),
    ...
    termStore.on('beforeload', function() {
    	Ext.apply(this.baseParams, {
            start: 0,
            equipTypeName: Ext.getCmp('equipTypeNamefind').getValue()
        });
    });
	
	var termColModel = new Ext.grid.ColumnModel([
    	new Ext.grid.RowNumberer(),
		{id: 'id',header: '终端类型id',dataIndex: 'id',sortable: true,width: 120,hidden : true},
		{header: '终端类型名称',dataIndex: 'equipTypeName',sortable: true,width: 120},
		{header: '终端类型状态',dataIndex: 'enableStatus',sortable: true,renderer:enableStatusRender,width: 120},
