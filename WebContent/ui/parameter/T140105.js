	...
	var delMenu = {
			...
			handler : function() {
				if(grid.getSelectionModel().hasSelection()) {
					...
					var toolUnitName = rec.get('toolUnitName');
					showConfirm('确定要删除该工具单位吗？工具单位名称：' + toolUnitName,grid,function(bt) {
						if(bt == "yes") {
							Ext.Ajax.requestNeedAuthorise({
								url: 'T140105Action.asp?method=delete',
								success: function(rsp,opt) {
//									alert(JSON.stringify(rsp)+"  >><<  "+JSON.stringify(opt));
									var rspObj = Ext.decode(rsp.responseText);
									if(rspObj == "00"){
										Ext.MessageBox.alert('操作提示', railConstOrgid+'工具单位已成功删除!');
										showSuccessMsg('工具单位已成功删除!',grid);//showErrorMsg(action.result.msg,addMhForm)
	            ...
	            Ext.Ajax.request({
					...
					success : function(rsp, opt) {
						...
						if(rspObj.success) {
				            ...
				            Ext.getCmp('toolUnitNameUp').setValue(rspObj.msg.toolUnitName);
	            ...
	            handler: function() {
	            	if(updTermForm.getForm().isValid()) {
						...
						updTermForm.getForm().submit({
							url: 'T140105Action.asp?method=edit',
							waitMsg: '正在保存，请稍后......',
							...
							params: {
								...
								toolUnitName: Ext.getCmp('toolUnitNameUp').getValue(),
								enableStatus:asVal,
								txnId: '140105',
								subTxnId: '02'
							}
		...
		var openMenu = {
				...
				handler : function() {
					if(grid.getSelectionModel().hasSelection()) {
						...
						var railConstOrgid = rec.get('id');
						var toolUnitName = rec.get('toolUnitName');
						showConfirm('确定要启用该工具单位吗？工具单位名称：' + toolUnitName,grid,function(bt) {
							if(bt == "yes") {
								Ext.Ajax.requestNeedAuthorise({
									url: 'T140105Action.asp?method=open',
									success: function(rsp,opt) {
//										Ext.MessageBox.alert('操作提示', '处理成功,已启动');
										showSuccessMsg('处理成功,已启动!',grid);//showErrorMsg(action.result.msg,addMhForm)
										grid.getStore().reload();
									},
									params: { 
										id:railConstOrgid,
		...
		var stopMenu = {
				...
				handler : function() {
					if(grid.getSelectionModel().hasSelection()) {
						...
						var railConstOrgid = rec.get('id');
						var toolUnitName = rec.get('toolUnitName');
						showConfirm('确定要停用该工具单位吗？工具单位名称：' + toolUnitName,grid,function(bt) {
							if(bt == "yes") {
								Ext.Ajax.requestNeedAuthorise({
									url: 'T140105Action.asp?method=close',
									success: function(rsp,opt) {
		            ...
		            Ext.Ajax.request({
						...
						success : function(rsp, opt) {
							...
							if(rspObj.success) {
					            ...
					            Ext.getCmp('toolUnitNameDet').setValue(rspObj.msg.toolUnitName);
	...
	var excelQueryForm = new Ext.form.FormPanel({
		...
		buttons: [{
			...
			handler: function() {
				...
				var toolUnitName = Ext.getCmp('toolUnitNamefind').getValue();
				var param = "?a=1";
				if(toolUnitName){
					param = param + "&toolUnitName="+toolUnitName;
				}
				window.location.href = Ext.contextPath +"/exportExcelT140105.asp"+param;
				excelDown.hide();
...
//			btn.disable();
			frm.submit({
				url: 'T140105Action.asp?method=add',
				waitTitle : '请稍候',
...
//	menuArr.push('-');
	...
	var termStore = new Ext.data.Store({
        proxy: new Ext.data.HttpProxy({
            url: 'gridPanelStoreAction.asp?storeId=toolUnit'
        }),
        reader: new Ext.data.JsonReader({
            ...
            {name: 'id',mapping: 'id'},
            {name: 'toolUnitName',mapping: 'toolUnitName'},
    ...
    termStore.on('beforeload', function() {
    	Ext.apply(this.baseParams, {
            start: 0,
            toolUnitName: Ext.getCmp('toolUnitNamefind').getValue()
        });
    });
	
	var termColModel = new Ext.grid.ColumnModel([
    	new Ext.grid.RowNumberer(),
		{id: 'id',header: '工具单位id',dataIndex: 'id',sortable: true,width: 120,hidden : true},
		{header: '工具单位名称',dataIndex: 'toolUnitName',sortable: true,width: 120},
		{header: '工具单位状态',dataIndex: 'enableStatus',sortable: true,renderer:enableStatusRender,width: 120},
