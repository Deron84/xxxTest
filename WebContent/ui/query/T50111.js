Ext.onReady(function() {
	...
	var gridStore = new Ext.data.Store( {
		proxy : new Ext.data.HttpProxy( {
			url : 'gridPanelStoreAction.asp?storeId=tblRTxnInfoAll'
		}),
...
//	                                           rexpander,
	...
	var forceMenu = {
			...
			handler : function() {
				...
				showConfirm('确定要强制发起该条重复退货信息吗？', grid, function(bt) {
					if (bt == 'yes') {
						// showProcessMsg('正在提交，请稍后......');
						var rec = grid.getSelectionModel().getSelected();
						Ext.Ajax.requestNeedAuthorise( {
							url : 'T90101Action.asp?method=force',
							method : 'post',
							params : {
...
//			plugins : rexpander,
		...
		gridStore.on('beforeload', function() {
			Ext.apply(this.baseParams,
					{
						start : 0,
						mchtId : topQueryPanel.getForm().findField('mchtCd')
								.getValue(),
						...
						txnAmt : topQueryPanel.getForm().findField('txnAmtQ')
								.getValue(),
						cardType : topQueryPanel.getForm().findField('cardType')
								.getValue(),
...
//		gridStore.load();
		...
		grid.getSelectionModel().on( {
			'rowselect' : function() {
				...
				var ms = rec.get('MATCH_STA');
				var ss = rec.get('OPR_STA');
				var as = rec.get('SETTLE_STATUS');
				
//				if ((ms == '10' &&  ss== '1'  && as=='1')//重复且匹配单笔,异常，待清算
//					 || (ms == '11' && ss == '1' && as=='1')) {//重复且匹配多笔，异常，待清算
