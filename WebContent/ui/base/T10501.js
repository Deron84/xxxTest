Ext.onReady(function() {
	...
	var dialog = new UploadDialog({
		uploadUrl : 'T10501Action.asp?method=batch',
		filePostName : 'files',
	...
	var gridStore = new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({
			url: 'gridPanelStoreAction.asp?storeId=posCardInfo'
		}),
		...
		handler: function() {
			if(grid.getSelectionModel().hasSelection()) {
				...
				showConfirm('确定要删除该条信息吗？',grid,function(bt) {
					if(bt == "yes") {
						Ext.Ajax.request({
							url: 'T10501Action.asp?method=delete',
	...
	var upMenu = {
		...
		handler: function() {
			...
			Ext.Ajax.request({
				url: 'T10501Action.asp?method=update',
				params: {
			...
			handler: function() {
				if(infoForm.getForm().isValid()) {
					infoForm.getForm().submitNeedAuthorise({
						url: 'T10501Action.asp?method=add',
						waitMsg: '正在提交，请稍后......',
