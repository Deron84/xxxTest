Ext.onReady(function() {
	var gridStore = new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({
			url: 'gridPanelStoreAction.asp?storeId=mchtAwaitAuditInf'
		}),
	...
	var colModel = new Ext.grid.ColumnModel([
	    ...
	    {header: '商户地址',dataIndex: 'addr',width: 140},
	...
	var acceptMenu = {
			...
			handler: function() {
				if(grid.getSelectionModel().hasSelection()) {
					...
					var planCheckDateArray=new Array();
					for(var i=0;i<rec.length;i++){
						mchtArray[i]=rec[i].get('mchtNo');
						planCheckDateArray[i]=rec[i].get('planCheckDate');
					}
					showConfirm('确定通过手动巡检吗？',grid,function(bt) {
						if(bt == "yes") {
							Ext.Ajax.request({
								url: 'T20802Action.asp?method=accept',
...
//									mchtNo: rec.get('mchtNo'),
									mchtArray:mchtArray,
									planCheckDateArray:planCheckDateArray,
									txnId: '20802',
									subTxnId: '01'
								}
							});
	...
	var refuseMenu = {
			...
			handler: function() {
				if(grid.getSelectionModel().hasSelection()) {
					...
					showConfirm('确定拒绝手动巡检吗？',grid,function(bt) {
						if(bt == "yes") {
							Ext.Ajax.request({
								url: 'T20802Action.asp?method=refuse',
								success: function(rsp,opt) {
									var rspObj = Ext.decode(rsp.responseText);
									if(rspObj.success) {
										showSuccessMsg(rspObj.msg,grid);
