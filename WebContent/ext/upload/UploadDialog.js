		...
		constructor : function(config) {
			...
			this.fileList = new Ext.grid.GridPanel({
				...
				columns : [new Ext.grid.RowNumberer(),
			        {header: '文件名',width : 100,dataIndex : 'fileName',sortable:false,renderer : this.formatFileName,id : autoExpandColumnId},
			        {header: '大小', width : 80,dataIndex:'fileSize',sortable:false,renderer : this.formatFileSize,align:'right'},
			        {header: '类型', width : 60,dataIndex : 'fileType',sortable:false,renderer : this.formatIcon,align:'center'},
			        {header: '进度', width : 100,dataIndex : '',sortable : false,renderer : this.formatProgressBar,align:'center'},
			        {
			        	header: '状态;',
			        	width : 100,
			        	dataIndex : 'fileState',
...
//					returnValue = '<span id="' + record.id + '"><div id="fileId_' + record.data.fileId + '" class="ux-cell-icon-delete"/></span>';
					returnValue = '<span id="' + record.id + '"><div id="fileId_' + record.data.fileId + '">等待上传</div></span>';
					break;
...
//					returnValue = '<span id="' + record.id + '"><div id="fileId_' + record.data.fileId + '" class="ux-cell-icon-clear"/></span>';
					returnValue = '<span id="' + record.id + '"><div id="fileId_' + record.data.fileId + '">已取消</div></span>';
					break;
				case SWFUpload.FILE_STATUS.COMPLETE:
//					returnValue = '<span id="' + record.id + '"><div id="fileId_' + record.data.fileId + '" class="ux-cell-icon-completed"/></span>';
					returnValue = '<span id="' + record.id + '"><div id="fileId_' + record.data.fileId + '">上传完成</div></span>';
					break;
				default : 
					alert('没有设置图表状态');
					break;
...
//			var bytes_added = completeBytes - thiz.progressInfo.currentCompleteBytes;
			var bytes_added = bytesTotal - thiz.progressInfo.currentCompleteBytes;
			if(k==1){
				thiz.progressInfo.bytesUploaded += filesize;
			}
			if(thiz.progressInfo.currentCompleteBytes>0){
				thiz.progressInfo.bytesUploaded += filesize;
			}
//			thiz.progressInfo.bytesUploaded += Math.abs(bytes_added < 0 ? 0 : bytes_added);
			console.log(bytes_added,bytesTotal,thiz.progressInfo.currentCompleteBytes,thiz.progressInfo.bytesUploaded);
//			thiz.progressInfo.currentCompleteBytes = completeBytes;
		...
		onUploadSuccess : function(file, serverData){
			...
			if(Ext.util.JSON.decode(serverData).success){
				...
				if(msgs == ''){
					...
					setTimeout(function(){Ext.MessageBox.hide();},timeout);
				}else{
					Ext.MessageBox.show({
						msg: msgs,
						title: '成功',
	...
	var dialog = new UploadDialog({
		...
		flashUrl : 'ext/upload/swfupload.swf',
		fileSize : '1000 MB', 
		fileTypes : '*.*',
